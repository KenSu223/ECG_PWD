    ...
    def train_step(self, real_doppler):
        ...
        # Train the discriminator
            with tf.GradientTape() as tape:
                ...
                real_logits = self.discriminator(keras.layers.Reshape((800, 1))(real_doppler), training=True)
                ...
                gp = self.gradient_penalty(batch_size, keras.layers.Reshape((800, 1))(real_doppler), generated_doppler)
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        np.save('./WGAN-GP_noise/logs/discriminator_losses.npy', np.array(self.d_losses))
        np.save('./WGAN-GP_noise/logs/generator_losses.npy', np.array(self.g_losses))
        np.save('./WGAN-GP_noise/logs/discriminator_accuracies.npy', np.array(self.d_accuracies))
        np.save('./WGAN-GP_noise/logs/generator_accuracies.npy', np.array(self.g_accuracies))
...
class CustomCallback(keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        ...
        np.save('./WGAN-GP_noise/logs/discriminator_losses.npy', d_losses)
        np.save('./WGAN-GP_noise/logs/generator_losses.npy', g_losses)
        np.save('./WGAN-GP_noise/logs/discriminator_accuracies.npy', d_accuracies)
        np.save('./WGAN-GP_noise/logs/generator_accuracies.npy', g_accuracies)

    def on_train_end(self, logs=None):
        # Save the final generator and discriminator models
        self.model.generator.save('./WGAN-GP_noise/models/final_generator.h5')
        self.model.discriminator.save('./WGAN-GP_noise/models/final_discriminator.h5')
...
plt.legend()
plt.savefig('WGAN-GP_noise/plots/loss-acc.png')
plt.show()
...
generated_sample = wgan.generator(random_latent_vectors).numpy().reshape(4,800)
