"""Composite-loss fusion training script with per-epoch loss-component decomposition and detailed logging artifacts."""
...
class LossComponentLogger(Callback):
    ...
    def __init__(self, X_train, Y_train, X_val, Y_val, alpha_peak, alpha_ratio, alpha_derivative=None, base_loss='mae'):
        super().__init__()
        ...
        if alpha_derivative is not None:
            self.history['train_derivative'] = []
            self.history['val_derivative'] = []
    ...
    def compute_components(self, y_true, y_pred):
        ...
        components = {
            'base': float(base.numpy()),
            'peak': float(peak_loss.numpy()),
            'ratio': float(ratio_loss.numpy())
        }
        
        # 4. Derivative loss (if applicable)
        if self.alpha_derivative is not None:
            dy_true = y_true[:, 1:] - y_true[:, :-1]
            dy_pred = y_pred[:, 1:] - y_pred[:, :-1]
            derivative_loss = tf.reduce_mean(tf.abs(dy_true - dy_pred))
            components['derivative'] = float(derivative_loss.numpy())
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        train_total = train_components['base'] + \
                     self.alpha_peak * train_components['peak'] + \
                     self.alpha_ratio * train_components['ratio']
        val_total = val_components['base'] + \
                   self.alpha_peak * val_components['peak'] + \
                   self.alpha_ratio * val_components['ratio']
        ...
        print(f"{'='*70}")
        print(f"{'Component':<20} {'Train':<15} {'Val':<15} {'Weight':<10}")
        print(f"{'-'*70}")
        print(f"{'Base (MAE/MSE)':<20} {train_components['base']:<15.6f} {val_components['base']:<15.6f} {'1.0':<10}")
        print(f"{'Peak Loss':<20} {train_components['peak']:<15.6f} {val_components['peak']:<15.6f} {self.alpha_peak:<10.2f}")
        print(f"{'Ratio Loss':<20} {train_components['ratio']:<15.6f} {val_components['ratio']:<15.6f} {self.alpha_ratio:<10.2f}")
        
        if self.alpha_derivative is not None:
            print(f"{'Derivative Loss':<20} {train_components['derivative']:<15.6f} {val_components['derivative']:<15.6f} {self.alpha_derivative:<10.2f}")
        
        print(f"{'-'*70}")
        print(f"{'TOTAL':<20} {train_total:<15.6f} {val_total:<15.6f}")
        print(f"{'='*70}\n")
...
def main(
    ...
    # Add loss component logger for composite/shape_preserving losses
    if loss_type in ['composite', 'shape_preserving']:
        alpha_deriv = alpha_derivative if loss_type == 'shape_preserving' else None
        loss_logger = LossComponentLogger(
            X[train_idx], Y[train_idx],
            X[val_idx], Y[val_idx],
            alpha_peak=alpha_peak,
            alpha_ratio=alpha_ratio,
            alpha_derivative=alpha_deriv,
            base_loss='mae'
        )
    ...
    # Save individual loss components if they were logged
    if loss_type in ['composite', 'shape_preserving']:
...
if __name__ == "__main__":
    ...
    #python train_fusion_wavenet_composite_log.py --loss_type shape_preserving --alpha_peak 0.2 --alpha_ratio 0.2 --alpha_derivative 0.2 --epochs 30