"""Reusable Keras layer wrappers for the single-channel WaveNet experiments. Keeps architecture code in `modules.py` concise and consistent."""
...
def CausalConv1D(filters, kernel_size,dilation_rate, strides=1, padding='causal', activation=None, use_bias = True):
    ...
    layer = keras.layers.Conv1D(
        filters=filters, kernel_size=kernel_size, dilation_rate=dilation_rate, strides=strides, padding=padding, data_format=None,
        groups=1, activation=activation, use_bias=use_bias, kernel_initializer=weights_initializer,
        bias_initializer="zeros", kernel_regularizer=None, bias_regularizer=None, activity_regularizer=None,
        kernel_constraint=None, bias_constraint=None
    )
