"""Signal-quality and morphology metric helpers (e.g., MAE, KLD, DTW, spectral metrics) used for waveform evaluation workflows."""
...
# ===============================
# 1. CALCULATE_KLD - MAJOR ISSUE
# ===============================
# Problem: Uses separate histograms for each signal, which is incorrect for KLD
# KLD should compare distributions element-wise or use proper probability distributions

def calculate_kld_fixed(predicted_signals, true_signals):
    """
    FIXED: Calculate KLD properly for signal comparison.
    Option 1: Element-wise probability distributions
    """
    predicted_signals = np.array(predicted_signals)
    true_signals = np.array(true_signals)
    
    # Flatten for overall comparison
    pred_flat = predicted_signals.flatten()
    true_flat = true_signals.flatten()
    
    # Create probability distributions with same bins
    bins = np.linspace(min(pred_flat.min(), true_flat.min()), 
                      max(pred_flat.max(), true_flat.max()), 50)
    ...
    # Calculate KLD: sum(P * log(P/Q))
    kld = np.sum(true_hist * np.log(true_hist / pred_hist))
    return kld

# Alternative: Per-sample KLD then average
def calculate_kld_per_sample(predicted_signals, true_signals):
    """
    Calculate KLD per sample and return average
    """
    kld_values = []
    for pred, true in zip(predicted_signals, true_signals):
        # Same logic as above but per sample
        bins = np.linspace(min(pred.min(), true.min()), 
                          max(pred.max(), true.max()), 30)
        pred_hist, _ = np.histogram(pred, bins=bins, density=True)
        true_hist, _ = np.histogram(true, bins=bins, density=True)
        
        pred_hist = pred_hist / np.sum(pred_hist) + 1e-10
        true_hist = true_hist / np.sum(true_hist) + 1e-10
...
# ===============================
# 2. CALCULATE_DTW - SHAPE ISSUE
# ===============================
# Problem: Expects 1D signals but will receive (X, 1065)
# Need to compute DTW per sample pair

def calculate_dtw_fixed(predicted_signals, true_signals):
    """
    FIXED: Calculate DTW for each sample pair and return average
    """
    from fastdtw import fastdtw
    from scipy.spatial.distance import euclidean
...
# ===============================
# 3. CALCULATE_SOFT_DTW - SHAPE ISSUE
# ===============================
# Problem: Current implementation assumes specific tensor shapes
# Need to handle (X, 1065) properly

def calculate_soft_dtw_fixed(predicted_signals, true_signals, gamma=1.0, use_cuda=False):
    """
    FIXED: Handle (X, 1065) shape properly for Soft-DTW
    """
    import torch
    from soft_dtw_cuda import SoftDTW
    
    # Convert to tensors and ensure proper shape
    X = torch.as_tensor(predicted_signals, dtype=torch.float32)  # (X, 1065)
    Y = torch.as_tensor(true_signals, dtype=torch.float32)      # (X, 1065)
    
    # SoftDTW expects (batch, time, features)
    # Our signals are (batch, time) so add feature dimension
    if X.ndim == 2:
        X = X.unsqueeze(-1)  # (X, 1065, 1)
        Y = Y.unsqueeze(-1)  # (X, 1065, 1)
    
    # Create Soft-DTW operator
    sdtw = SoftDTW(gamma=gamma, use_cuda=use_cuda)
    
    # Calculate distances - this will return (X,) for X sample pairs
    dist = sdtw(X, Y)
    
    return dist.mean().item()

# ===============================
# 4. SPECTRAL FUNCTIONS - POTENTIAL ISSUES
# ===============================
# These functions work on single signals but need batch processing

def calculate_spectral_centroid_batch(signals, fs=284):
    """
    Calculate spectral centroid for batch of signals
    """
    centroids = []
...
def calculate_spectral_entropy_fixed(generated_signals, real_signals, fs=284):
    """
    FIXED: Calculate spectral entropy difference for batch signals
    """
    import scipy.signal
    ...
    for gen_sig, real_sig in zip(generated_signals, real_signals):
        # Generated signal entropy
        _, Pxx_gen = scipy.signal.welch(gen_sig, fs=fs)
        Pxx_gen_normalized = Pxx_gen / np.sum(Pxx_gen)
        entropy_gen = -np.sum(Pxx_gen_normalized * np.log2(Pxx_gen_normalized + 1e-10))
        gen_entropies.append(entropy_gen)
        
        # Real signal entropy  
        _, Pxx_real = scipy.signal.welch(real_sig, fs=fs)
        Pxx_real_normalized = Pxx_real / np.sum(Pxx_real)
        entropy_real = -np.sum(Pxx_real_normalized * np.log2(Pxx_real_normalized + 1e-10))
        real_entropies.append(entropy_real)
    
    return np.mean(np.abs(np.array(gen_entropies) - np.array(real_entropies)))

# ===============================
# 5. DISCRETE FRECHET DISTANCE - SHAPE ISSUE  
# ===============================
# Problem: Expects 1D sequences but will receive (X, 1065)

def calculate_discrete_frechet_distance_batch(predicted_signals, true_signals):
    """
    Calculate discrete Frechet distance for each sample pair
    """
    distances = []
    for pred, true in zip(predicted_signals, true_signals):
        # Reshape to column vectors for point-wise distance calculation
        E = pred.reshape(-1, 1)
        E_prime = true.reshape(-1, 1) 
        fd = calculate_discrete_frechet_distance(E, E_prime)
        distances.append(fd)
...
def calculate_kld(predicted_signals, true_signals):
    ...
    pred_distribution = np.histogram(predicted_signals, bins=30, density=True)[0]
    true_distribution = np.histogram(true_signals, bins=30, density=True)[0]
...
def calculate_soft_dtw(predicted_signals, true_signals, gamma=1.0, use_cuda = False):
    """
    ...
    """
    # Ensure PyTorch tensors with an explicit batch dimension
    X = torch.as_tensor(predicted_signals, dtype=torch.float32)
    Y = torch.as_tensor(true_signals,    dtype=torch.float32)
    if X.ndim == 2:            # (T, D) ➜ (1, T, D)
        X = X.unsqueeze(0)
        Y = Y.unsqueeze(0)

    # Create the differentiable Soft-DTW operator
    sdtw = SoftDTW(gamma=gamma,
                   dist_func=pairwise_l2_squared,
                   use_cuda=use_cuda) 

    # Forward call returns a tensor of shape (B,)
    dist = sdtw(X, Y)

    # Return scalar for single pair, or mean over batch
    return dist.mean().item()
...
def calculate_spectral_centroid(signal, fs=284):
    """
...
def calculate_spectral_entropy(generated_signals, real_signals, fs=284):
    """
...
def calculate_psd_difference(generated_signals, real_signals, fs=284):
    """
...
def calculate_spectral_centroid_difference(generated_signals, real_signals, fs=284):
    """
