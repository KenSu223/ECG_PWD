...
def multi_resolution_stft_loss(y_true, y_pred, fft_sizes=[256, 512, 1024], hop_sizes=[64, 128, 256], win_lengths=[256, 512, 1024]):
    """
...
def create_composite_loss(alpha_peak=0.5, alpha_ratio=0.5, base_loss='mae'):
    """Create composite loss function for WaveNet training (legacy)"""
...
def create_shape_preserving_loss(alpha_peak=0.5, alpha_ratio=0.5, alpha_derivative=0.3, base_loss='mae'):
    """Enhanced composite loss with derivative matching (legacy)"""
    return create_flexible_loss(
...
def main(
    ...
    if loss_type == 'flexible':
        ...
        loss_fn = create_composite_loss(alpha_peak=alpha_peak, alpha_ratio=alpha_ratio, base_loss=base_loss)
        ...
        use_softdtw, use_corr, use_mrstft = False, False, False
        print(f"✓ Using shape-preserving loss")
    ...
    # Visualization code
    print("\n" + "="*80)
    print("Generating visualization plots for random samples from BEST MODEL")
    print("="*80)
    ...
    np.random.seed(33)
    random_indices = np.random.choice(val_idx, size=min(3, len(val_idx)), replace=False)
    ...
    print("\n" + "-"*80)
    print("Plotting samples from FINAL model...")
    print("-"*80)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train WaveNet model with flexible loss components")
    ...
    parser.add_argument("--train_idx_file", type=str, default="/home/tsu25/ECG_PWD/Fetal-maternal-fusion/src/idx_train_mix_risk.npy")
    parser.add_argument("--val_idx_file", type=str, default="/home/tsu25/ECG_PWD/Fetal-maternal-fusion/src/idx_val_mix_risk.npy")
