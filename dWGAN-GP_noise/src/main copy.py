...
dataset = tf.data.Dataset.from_tensor_slices((DUS_array_train))
dataset = dataset.shuffle(buffer_size=4096).batch(batch_size)



class dWGAN(keras.Model):
    def __init__(self,  discriminator_2, generator, latent_dim, gp_weight_2):
        super().__init__()
    ...
    def compile(self,  d2_optimizer, g_optimizer,d2_loss_fn, g_loss_fn):
        super().compile()
    ...
    @property
    def metrics(self):
        return [self.d2_loss_metric,self.g_loss_metric]
    ...
    def train_step(self, real_doppler):
        ...
        # Train the discriminator
            with tf.GradientTape(persistent=True) as tape:
                ...
                a = keras.layers.Reshape((800, 1))(generated_doppler)

                scalograme_generated_doppler = modules.create_batch_scalograms(a)
                #print(scalograme_generated_doppler)


                fake_logits_2 = self.discriminator_2(scalograme_generated_doppler, training=True)

                real_logits_2 = self.discriminator_2(keras.layers.Reshape((80, 160, 1))(modules.create_batch_scalograms(keras.layers.Reshape((800, 1))(real_doppler))), training=True)

                d2_cost = self.d2_loss_fn(real_doppler=real_logits_2, fake_doppler=fake_logits_2)

                gp_2 = self.gradient_penalty_2(batch_size, keras.layers.Reshape((80, 160, 1))(modules.create_batch_scalograms(keras.layers.Reshape((800, 1))(real_doppler))), keras.layers.Reshape((80, 160, 1))(modules.create_batch_scalograms(generated_doppler)))
        ...
        with tf.GradientTape() as tape:
            ...
            gen_doppler_logits = self.discriminator_2(keras.layers.Reshape((80, 160, 1))(modules.create_batch_scalograms(generated_doppler)), training=True)
...
class CustomCallback(keras.callbacks.Callback):
    ...
    def on_train_end(self, logs=None):
        ...
        self.model.discriminator.save('./dWGAN-GP_noise/models/final_discriminator_1.h5')
        self.model.discriminator.save('./dWGAN-GP_noise/models/final_discriminator_2.h5')
...
dwgan = dWGAN(
discriminator_2=modules.discriminator_2((80,160,1)),
    generator=modules.generator(latent_dim), 
    latent_dim=latent_dim, gp_weight_2=20
    )
...
plt.subplot(1, 2, 1)
plt.plot(epochs_range, d1_losses, label='Discriminator Loss')
plt.plot(epochs_range, d2_losses, label='Discriminator Loss')
