...
DUS_array = np.array(DUS_list, dtype=np.float32).reshape(5103,800,1)
...
class GAN(keras.Model):
    ...
    def train_step(self, real_doppler):
        ...
        combined_doppler = tf.concat([generated_doppler, keras.layers.Reshape((800, 1))(real_doppler)], axis=0)
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        np.save('./LSTM_GAN_noise/logs/discriminator_losses.npy', np.array(self.d_losses))
        np.save('./LSTM_GAN_noise/logs/generator_losses.npy', np.array(self.g_losses))
        np.save('./LSTM_GAN_noise/logs/discriminator_accuracies.npy', np.array(self.d_accuracies))
        np.save('./LSTM_GAN_noise/logs/generator_accuracies.npy', np.array(self.g_accuracies))
...
class CustomCallback(keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        ...
        np.save('./LSTM_GAN_noise/logs/discriminator_losses.npy', d_losses)
        np.save('./LSTM_GAN_noise/logs/generator_losses.npy', g_losses)
        np.save('./LSTM_GAN_noise/logs/discriminator_accuracies.npy', d_accuracies)
        np.save('./LSTM_GAN_noise/logs/generator_accuracies.npy', g_accuracies)

    def on_train_end(self, logs=None):
        # Save the final generator and discriminator models
        self.model.generator.save('./LSTM_GAN_noise/models/final_generator.h5')
        self.model.discriminator.save('./LSTM_GAN_noise/models/final_discriminator.h5')
...
plt.legend()
plt.savefig('LSTM_GAN_noise/plots/loss-acc.png')
plt.show()

# Randomly select 4 ECG samples and their corresponding real Doppler images from your dataset
random_latent_vectors = tf.random.normal(shape=(4, 800), seed=2024)
generated_sample = lstmgan.generator(random_latent_vectors).numpy().reshape(4,800)
