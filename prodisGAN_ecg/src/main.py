...
class prodisGANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        np.save('./prodisGAN_ecg/logs/discriminator_losses.npy', np.array(self.d_losses))
        np.save('./prodisGAN_ecg/logs/generator_losses.npy', np.array(self.g_losses))
        np.save('./prodisGAN_ecg/logs/discriminator_accuracies.npy', np.array(self.d_accuracies))
        np.save('./prodisGAN_ecg/logs/generator_accuracies.npy', np.array(self.g_accuracies))
...
class CustomCallback(keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        ...
        np.save('./prodisGAN_ecg/logs/discriminator_losses.npy', d_losses)
        np.save('./prodisGAN_ecg/logs/generator_losses.npy', g_losses)
        np.save('./prodisGAN_ecg/logs/discriminator_accuracies.npy', d_accuracies)
        np.save('./prodisGAN_ecg/logs/generator_accuracies.npy', g_accuracies)

    def on_train_end(self, logs=None):
        # Save the final generator and discriminator models
        self.model.generator.save('./prodisGAN_ecg/models/final_generator.h5')
        self.model.discriminator.save('./prodisGAN_ecg/models/final_discriminator.h5')
...
for i in range(enhancement_stages+1):
    if i == 0: 
        ...
        discriminator = modules.load_trained_model('./prodisGAN_ecg/models/final_discriminator.h5')
        generator = modules.load_trained_model('./prodisGAN_ecg/models/final_generator.h5')
        if i == 1:
            discriminator = modules.pro_discriminator1(discriminator)
        else: 
            discriminator = modules.pro_discriminator2(discriminator)
...
epochs_range = range(epochs*(enhancement_stages+1))
...
plt.legend()
plt.savefig('prodisGAN_ecg/plots/loss-acc.png')
plt.show()
...
generated_dopplers = prodisgan.generator(selected_ecgs).numpy().reshape(4,800)
