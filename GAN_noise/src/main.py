...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        np.save('./GAN_noise/logs/discriminator_losses.npy', np.array(self.d_losses))
        np.save('./GAN_noise/logs/generator_losses.npy', np.array(self.g_losses))
        np.save('./GAN_noise/logs/discriminator_accuracies.npy', np.array(self.d_accuracies))
        np.save('./GAN_noise/logs/generator_accuracies.npy', np.array(self.g_accuracies))
...
class CustomCallback(keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        ...
        np.save('./GAN_noise/logs/discriminator_losses.npy', d_losses)
        np.save('./GAN_noise/logs/generator_losses.npy', g_losses)
        np.save('./GAN_noise/logs/discriminator_accuracies.npy', d_accuracies)
        np.save('./GAN_noise/logs/generator_accuracies.npy', g_accuracies)

    def on_train_end(self, logs=None):
        # Save the final generator and discriminator models
        self.model.generator.save('./GAN_noise/models/final_generator.h5')
        self.model.discriminator.save('./GAN_noise/models/final_discriminator.h5')
...
plt.legend()
plt.savefig('GAN_noise/plots/loss-acc.png')
plt.show()
...
generated_sample = dcgan.generator(random_latent_vectors).numpy().reshape(4,800)
